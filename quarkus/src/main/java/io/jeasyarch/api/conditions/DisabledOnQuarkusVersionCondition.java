package io.jeasyarch.api.conditions;
...
import io.jeasyarch.api.DisabledOnQuarkusVersion;
...
public class DisabledOnQuarkusVersionCondition implements ExecutionCondition {
    private static final ConditionEvaluationResult ENABLED_BY_DEFAULT = ConditionEvaluationResult
            .enabled("@DisabledOnQuarkusVersion is not present");

    @Override
    public ConditionEvaluationResult evaluateExecutionCondition(ExtensionContext context) {
        Optional<AnnotatedElement> element = context.getElement();
        return AnnotationUtils.findRepeatableAnnotations(element, DisabledOnQuarkusVersion.class).stream()
                .filter(this::isDisabledOnCurrentQuarkusVersion).findAny().map(this::testIsDisabled)
                .orElse(ENABLED_BY_DEFAULT);
    }

    private ConditionEvaluationResult testIsDisabled(DisabledOnQuarkusVersion disabledOnQuarkus) {
        return ConditionEvaluationResult
                .disabled("Disabled on Quarkus version (reason: " + disabledOnQuarkus.reason() + ")");
    }
