...
public final class GitUtils {
    ...
    public static void cloneRepository(ServiceContext serviceContext, String repository) {
        try {
            new Command(Arrays.asList(GIT, CLONE, repository, ".")).outputToConsole()
                    .onDirectory(serviceContext.getServiceFolder().toString()).runAndWait();
    ...
    public static void checkoutBranch(ServiceContext serviceContext, String branch) {
        try {
            new Command(Arrays.asList(GIT, CHECKOUT, branch)).outputToConsole()
                    .onDirectory(serviceContext.getServiceFolder().toString()).runAndWait();
        } catch (Exception e) {
