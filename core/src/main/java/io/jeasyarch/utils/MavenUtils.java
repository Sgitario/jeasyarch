...
public final class MavenUtils {
    ...
    private static void installParentPom(Path relativePath) {
        List<String> args = new ArrayList<>(
                asList(MVN_COMMAND, DISPLAY_ERRORS, INSTALL_GOAL, SKIP_CHECKSTYLE, SKIP_TESTS, SKIP_ITS, "-pl", "."));
        withMavenRepositoryLocalIfSet(args);
        withProperties(args);
    ...
    private static void withProperties(List<String> args) {
        ...
        PropagatePropertiesStrategy strategy = PropagatePropertiesStrategy.ONLY_QUARKUS;

        System.getProperties().entrySet().stream().filter(strategy.isPropagated().and(propertyValueIsNotEmpty()))
                .forEach(property -> {
    ...
    private enum PropagatePropertiesStrategy {
        ALL("all", name -> true), NONE("none", name -> false),
        ...
        public static PropagatePropertiesStrategy fromValue(String value) {
            ...
            throw new IllegalArgumentException(
                    "Property `maven.propagate-properties-strategy` with invalid value: " + value);
        }
    }
}
